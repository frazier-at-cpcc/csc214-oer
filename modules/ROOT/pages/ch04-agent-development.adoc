...
=== 4.2.3 The Connection Lifecycle: Initialization, Capability Negotiation, Operation
...
*Foundry IQ* (part of Microsoft Foundry, generally available through the 2026-04-01 REST API) is a managed knowledge layer that an agent calls as a single, reasoning-driven knowledge source. Built on Azure AI Search, it takes over the retrieval machinery you assembled in Chapter 2 and adds a layer of reasoning on top of it.
...
[NOTE]
====
As with the MCP example, exact Foundry SDK symbols and the precise `tools` payload for a knowledge base vary by SDK and REST API version (Foundry IQ is GA via the 2026-04-01 REST API). The example shows the integration *shape* -- reference a named knowledge base, attach it as a tool -- not an authoritative API contract. Confirm the symbols against the SDK version in your lab environment.
====
